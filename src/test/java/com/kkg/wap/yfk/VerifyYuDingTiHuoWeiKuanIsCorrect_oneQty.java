...
public class VerifyYuDingTiHuoWeiKuanIsCorrect_oneQty {
    ...
    public void yuDingTiHuo()throws Exception {
        ...
        BigDecimal expectedTotalPrice= new BigDecimal("30088.91").setScale(2);
        BigDecimal expectedBuTie= new BigDecimal("30.00").setScale(2);
        BigDecimal expectedWeiKuan= new BigDecimal("30058.91").setScale(2);

        System.out.println("实际商品总额：" + p + "期望商品总额：" +expectedTotalPrice);
        System.out.println("实际提货补贴：" + t + "期望提货补贴：" +expectedBuTie);
        System.out.println("实际提货尾款：" + p + "期望提货尾款：" +expectedTotalPrice);
        //判断提货尾款正确
        Assert.assertEquals(p,expectedTotalPrice,"商品总额不正确！！！");
        Assert.assertEquals(t,expectedBuTie,"提货补贴不正确！！！");
        Assert.assertEquals(wk,expectedWeiKuan,"提货尾款不正确！！！");
